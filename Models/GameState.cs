...
namespace OnitamaTestClient.Models {
    public class GameState {
        ...
        public List<Piece> MyPieces => this.Pieces.Where(p => p.Owner == CurrentlyPlaying).ToList();
        ...
        public string PrintBoard() {
            ...
            for (var y = dimY - 1; y >= 0; y--) {
                ...
                for (var x = 0; x < dimX; x++) {
                    var piece = this.Pieces.Find(p => p.PositionOnBoard.X == x && p.PositionOnBoard.Y == y);
                    if (piece == null) {
                        stringBuilder.Append(" . ");
                        continue;
                    }

                    var marking = piece.Type == Enums.PieceTypeEnum.Pawn ? "p" : "k";
                    if (piece.Owner == this.CurrentlyPlaying) {
        ...
        public string PrintHand(List<Card> hand) {
            ...
            foreach (var card in hand) {
                stringBuilder.Append(this.PrintCard(card));
                stringBuilder.Append("\n");
            }
